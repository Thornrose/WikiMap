...
const addPoint = (req, res) => {
  ...
  const { point_title, point_description, point_image_url, lat, long } =
    req.body;
...
const update = (req, res) => {
  ...
  const latitude = req.body.point_latitude.slice(1);
  const longitude = req.body.point_longitude;
  const pointID = req.body.point_point_id;
  ...
  pointsDBmodel
    .update(title, description, imageUrl, latitude, longitude, id)
    .then((point) => {
      if (!point) {
        return res.status(404).send({ message: "Point not found!" });
      }
